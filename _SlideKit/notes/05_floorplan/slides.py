...
SPECS = [
    ...
    split("1.5 输入、输出与导入前提", "输入各有作用，输出须可继续用", "f12_io.png", [
        ...
        "输入·面积需求（area requirements）：估算核心区尺寸、利用率与宏单元空间",
        "输入·电源需求（power requirements）：决定电源焊盘、电源网格、高功耗宏单元位置",
        "输入·时序约束（timing constraints）：影响模块划分、引脚分配、宏单元数据流",
        "输入·物理分区（physical partitioning）：给层次化设计的模块边界与约束",
        "输入·IO / 宏单元提示：减少初期迭代成本",
        "输出·晶粒或模块面积已定，IO 已摆放，硬宏已摆放并固定",
        "输出·电源网格初步设计完成，必要的电源预布线（power pre-routing）已完成，标准单元可放置区已定义",
        "输出·阻挡 / 留边 / 区域约束已落入数据库，可布线性（routability）/ 时序 / 电源完整性（power integrity）已有早期评估",
    ], diagram="io"),
    ...
    split("2.4 硬宏摆放", "把大宏推到边角，留完整大矩形", "f06_macro.png", [
        "硬宏含 SRAM、ROM、PLL、模拟 IP、第三方 hard IP：面积大、形状 / 引脚固定，内部不可被摆放器（placer）任意拆开，对布线、时序、电源都有强影响",
        "主线：把大宏推到边缘或角落，让标准单元区域保持一个尽量完整的大矩形",
        "沿边 / 沿角：大宏靠核心区边界或角落，不在中心挖洞",
        "按数据流（dataflow）就近：让强连接的宏 / 逻辑靠近，用飞线（flyline）看连接权重",
        "保留布线通道：宏与宏、宏与边界之间留足信号、电源、时钟通道",
        "改善引脚可达性（pin accessibility）：必要时旋转宏，让引脚面向更开放区域，避免对着窄通道或角落",
        "摆好后固定：硬宏位置确定后通常标记为固定",
        "引线键合（wire-bond）下高功耗宏勿放中心太深——电源焊盘多在边缘，供电路径越长 IR 压力越大",
    ], style="num", figs=[
    ...
    split("2.6 阻挡与留边", "布局阻挡限制放置（三种），留边给宏通道留活路", "ext_blockage_floorset_ccby.png", [
        ...
        "软阻挡（soft blockage）初始布局阶段禁用，优化阶段可放缓冲器（buffer）/ 反相器（inverter）",
        "部分阻挡（partial blockage）限制区域的最大密度，例如最多 40%",
        "留边（halo，又叫 padding / keepout margin）是宏单元外围一圈空白，主要用于改善宏引脚可达性，并为缓冲器 / 反相器插入、电源与信号布线留近宏通道",
    ...
    split("4.7 电源网格与宏摆放", "高功耗宏要看供电路径", "ext_irlayout_cfirstnet_ccby.png", [
        ...
        "IR 压降通常用颜色图（color map）找热点（hot spot）；修复常是热点附近加电源线 / 加过孔阵列 / 加宽条带 / 移高功耗宏 / 增加或重分配电源焊盘",
