...
SPECS = [
    ...
    split("6.1 发展脉络", "四十年主线：从切片构造到固定轮廓与解析法", "f20_devthread.png", [
        P("布图规划在算法上要回答两件事：**拓扑生成（topology generation）定谁挨着谁，尺寸确定（sizing）定每块多大**；优化目标是面积、线长（wirelength）与长宽比（aspect ratio），模块分硬块（hard，形状固定）与软块（soft，可变形）两类。"),
        L("沿四十年发展，主线分为五个阶段："),
        N("__早期构造法（1980s）__：min-cut 划分、矩形对偶（rectangular dualization）、层次枚举，先区分可切片（slicible）与不可切片（non-slicible）布图"),
        N("__切片 + 模拟退火（1986, Wong–Liu）__：用切片树和 Polish 表达式编码，配 Stockmeyer 形状曲线与模拟退火搜索"),
        N("__非切片表示（1995–2005）__：Sequence Pair、B*-tree、TCG 等表示法登场，能表达切片树无法表达的紧致布局"),
        N("__现代布图规划（2000s）__：转向固定轮廓（fixed-outline），并引入解析法、拥塞 / 缓冲 / 时序 / 功耗感知"),
        N("__工业布图与原型（prototyping）__：在真实层次化设计里做可行性闸门与时序预算"),
        P("这条脉络的内在驱动力是**表示法越来越通用、优化目标从「最小面积」转向「塞进给定轮廓且各项指标可收敛」**。"),
    ...
    split("6.2c Wong–Liu 模拟退火", "在规范化 Polish 表达式上做三种局部扰动", "f23_wongliu.png", [
        ...
        P("每次移动只改变切片树的一小棵子树，所以**只需增量重算受影响子树的形状曲线**，不必从头打包，这让 SA 每步都很便宜——这是该方法实用的关键。"),
    ...
    split("6.3a Sequence Pair", "一对序列 → 约束图 → 最长路定坐标", "f24_seqpair.png", [
        ...
        P("据此建**水平约束图与垂直约束图**（加源点 / 汇点，边权为模块宽 / 高），各求一次**最长路径（longest path）**就得到所有模块坐标；朴素打包 O(n^2)，用最长公共子序列技巧可降到 O(n log log n)。"),
    ...
    split("6.3b B*-tree", "单棵有序二叉树 + contour 轮廓线，O(n) 打包", "f25_bstar.png", [
        ...
        P("沿深度优先（DFS）遍历一次即可定出全部 x 坐标；y 坐标借**轮廓线（contour）** 数据结构在线维护当前天际线，整体打包只需 **O(n)** 线性时间。"),
        P("B\\*-tree 打包快、实现简单，与 O-tree 等价；扰动用旋转 / 移动 / 交换三种操作。"),
    ], caption="一个 packing + B*-tree（根=左下，左孩=右邻 x_j=x_i+w_i，右孩=上邻 x_j=x_i）+ contour"),

    tbl("6.3c 表示法对比", "解空间越大越通用，但打包越慢、关系越不透明——按场景权衡",
    ...
    bl("6.4b 现代 floorplanning 要点", "目标从「压到最小」转为「在给定轮廓里把各项指标做好」", [
        "**解析 / 数学规划法**把布图写成可优化的数学问题（如 min-cost max-flow、非线性规划），在大规模问题上比纯 SA 更可扩展",
        "**空白（whitespace）当作资源**而非浪费：留给布线、缓冲器插入与时序裕量，而不是一味压到最小",
        "**拥塞感知（congestion-aware）**：用网格估计布线拥塞，把拥塞代价直接并入退火 / 优化目标，避免布局阶段才发现绕不出去",
        "**缓冲规划（buffer planning）**：为长互连预留缓冲器位置与通道，保证时序在布线后仍可收敛",
        "**吞吐 / 微架构感知**：吞吐率 ≈ 频率 / CPI，过长的流水互连会抬高 CPI，因此布图要兼顾微架构层面的数据通路长度",
        "**功耗感知（power-aware）**：把高功耗块摆在靠近电源焊盘处以降低 IR 压降，并为专用结构（FPGA 异构资源、模拟对称约束、3D 堆叠）保留约束",
    ]),

    split("6.5 工业布图与原型", "floorplanning 诊断可行性，prototyping 做签核前闸门", "f28_hier.png", [
        P("工业流程把这一步分成两段：**早期 floorplanning** 诊断可行性、容忍不完整或不一致的数据、欢迎人工介入；**后期原型（prototyping）** 验证 RTL 真能实现，是布线 / 时序 / 功耗签核前的可行性闸门。"),
        L("扁平（flat）与层次化（hierarchical）的取舍："),
        B("层次化靠**预算切块**把大设计拆开，但一个不合理的块预算就可能拖垮整片"),
        B("工业常用做法：先 flat 摆布线一次，在跨边界处定引脚，保证可行后再切块"),
        P("配套手段包括**引脚分配 + 时序预算**——零-slack 算法（Hauge）把松弛分配到各段、定出必需到达时间；以及**可布线性分析（routability）**——靠全局布线或试布线提前估拥塞，再叠加功耗网络与 ECO replay。"),
