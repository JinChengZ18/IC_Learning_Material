...
SPECS = [
    ...
    split("2.1 IO 环与芯片尺寸", "由外到内 + 核心受限 / 焊盘受限", "ext_die_i9_cc0.jpg", [
        ...
        B("焊盘受限（pad limited）：IO 数量、焊盘间距（pad pitch）与封装引脚分配决定、晶粒已不能再小，重点优化 IO 环、焊盘 / 凸点分配与封装协同"),
    ], figs=[
    ...
    split("3.3 引脚分配与穿通", "顶层 IO 与模块引脚影响点不同，穿通是层次化关键", "ext_wirebond_ccbysa.jpg", [
        ...
        B("顶层 IO：影响封装、静电防护（ESD）、电源焊盘与芯片边界"),
        B("模块引脚：影响模块间布线、时序预算、穿通（feedthrough）与全芯片收敛"),
