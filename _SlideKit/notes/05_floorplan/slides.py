...
SPECS = [
    ...
    split("1.1 从逻辑网表到物理骨架", "进入物理世界的第一层承诺", "ext_die_486_ccby.jpg", [
        "综合后的门级网表（gate-level netlist）只说明实例（instance）与网络（net），不含单元、宏、IO 或电源结构的物理位置",
        "布图规划（floorplan）把这张逻辑网表落成可实现的物理骨架——从逻辑世界进入物理世界的第一层承诺",
        "骨架至少含：晶粒（die）边界与核心区（core），以及 IO / 焊盘（pad）/ 凸点（bump）的大致位置",
        "还含硬宏（hard macro）的位置、朝向与固定状态，以及标准单元可放置区（standard-cell placement area）",
        "并落定电源环（power ring）/ 条带（stripe）/ 网格（mesh）/ 供电轨，阻挡（blockage）/ 留边（halo）/ 区域（region）约束",
        "后续布局、CTS、布线与电源签核都沿此骨架细化；此阶段改成本低，签核再改则牵一发而动全身",
    ], figs=[
    ...
    ], note="这些对象互相抢空间：IO 在外圈、核心区在内部、宏单元切割标准单元区、电源网格又要把电送到每个单元；好 floorplan 让它们抢得有序。这也是 Floorplan 重要的原因——修改成本随实现阶段上升，早期定好骨架，才能在面积 / 性能 / 功耗（PPA）之间做可承受的权衡。"),
    split("1.5 输入、输出与导入前提", "输入各有作用，输出须可继续用", "f12_io.png", [
        "输入·设计网表（design netlist）：提供实例与连接关系",
        "输入·面积需求（area）：估算核心区尺寸、利用率与宏单元空间",
        "输入·电源需求（power）：决定电源焊盘、电源网格、高功耗宏位置",
        "输入·时序约束（timing）：影响模块划分、引脚分配、宏单元数据流",
        "输入·物理分区（partition）：给层次化的模块边界与约束；IO / 宏提示可选，省初期迭代",
        "输出·晶粒 / 模块面积已定，IO 已摆放，硬宏已摆放并固定",
        "输出·电源网格初步设计、必要电源预布线（pre-route）完成，标准单元可放置区已定义",
        "输出·阻挡 / 留边 / 区域约束入库，可布线性（routability）/ 时序 / 电源完整性已有早期评估",
    ], diagram="io"),
    ...
    split("2.6 阻挡与留边", "布局阻挡限制放置（三种），留边给宏通道留活路", "ext_blockage_floorset_ccby.png", [
        "布局阻挡（placement blockage）限制标准单元摆放，按强度分硬、软、部分三种",
        "硬阻挡（hard）完全禁止放置标准单元",
        "软阻挡（soft）初始布局阶段禁用，优化阶段可放缓冲器（buffer）/ 反相器（inverter）",
        "部分阻挡（partial）限制区域的最大密度，例如最多 40%",
        "留边（halo，又叫 padding / keepout）是宏外围一圈空白，改善引脚可达性，并为缓冲器 / 反相器插入、电源与信号布线留近宏通道",
        "关键区别：留边（halo）跟随宏单元移动，而阻挡（blockage）多为固定坐标区域",
    ...
    split("2.7 布线阻挡与好 Floorplan", "用途清单 + 好 floorplan 的图上可见特征", "f07_halo.png", [
        "布线阻挡（routing blockage）限制走线而非单元摆放，可指定布线层（routing layer）或层范围，例如禁 M1–M3 在某区域走线",
        "用途·保护宏单元上方的特殊区域，并为电源结构预留布线资源",
        "用途·避免低层金属被不合适的信号占用，保护噪声敏感的模拟区",
        "好 floorplan：保持单个大的核心放置区，RAM 这类大块放在角落或边缘",
        "好 floorplan：保留宽布线通道，避免收缩出狭窄通道，不让大量引脚对着窄通道",
        "好 floorplan：必要时旋转宏单元改善引脚可达性，引脚不挤在模块角落",
        "好 floorplan：用阻挡 / 留边在早期就提前保护可布线性（routability）",
    ...
    split("3.3 引脚分配与穿通", "顶层 IO 与模块引脚影响点不同，穿通是层次化关键", "ext_wirebond_ccbysa.jpg", [
        "顶层 IO 摆放决定芯片对外接口位置，影响封装、静电防护（ESD）、电源焊盘与芯片边界；模块级引脚分配（pin assignment）则决定层次化模块之间如何连接，影响模块间布线、时序预算、穿通（feedthrough）与全芯片收敛。",
    ...
    split("4.1 电源规划：功耗与可靠性", "PDN 不只是连上 VDD/GND", "ext_pdn_bitfury_ccby.jpg", [
        "电源规划要解决的不只是“连上 VDD/GND”，它同时关系到动态功耗（dynamic power）、静态漏电（static leakage）、IR 压降（IR drop）、电压跌落（voltage drop）、电迁移（EM）与自热磨损（self-heating）。",
        "电源分布网络（PDN）需要把电流从焊盘 / 凸点送到晶体管，保持稳定低噪声的电压，提供平均与峰值功耗，为信号提供返回路径，避免电迁移与自热，并合理占用面积与布线资源。",
        "因此电源规划从 floorplan 阶段就开始，而不是等布线之后再补救。右图是一颗真实芯片的顶层金属，几乎整层都用于供电网。",
    ...
    ], note="判断标准：好 floorplan 要让下游布局（placement）/ CTS / 布线（routing）/ 电源签核（signoff）都有路可走。工作方式是迭代闭环：布图规划 → 试布局 / 试布线 → 拥塞 / 时序 / IR 早期分析 → 调利用率 / 宏 / 通道 / 引脚 / PG → 重复直到可接受 → 进入详细布局。"),
