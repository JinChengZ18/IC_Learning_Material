...
- **不滥用分点（多次返工的坑）**：**体裁逐段对齐笔记**——一页内可**混合体裁**：散文段用 `P("…")`、引导句用 `L("…：")`（均无标记整段），并列项 `B("…")`(▪)、有序项 `N("…")`(①，仅 N 计数)。笔记是「散文 + 列表 + 散文」的小节就照搬这个结构（如 1.1/2.4/2.6/2.7/4.3/4.7），**别把散文框架也分点、别给非列表项编号**。纯散文小节整页 `P`（或 `style="prose"`）。对齐**不只是文字、也包含分点格式**。
...
- **纯表格页别只有表**：笔记里表前后的引言 / 定义 / 结论用 `tbl(..., note="…")` 补到表下（`deck._table` 自动缩表留位、≤2 行）；表格列 / 行按笔记给足。
...
## 真实插图流水线（**只用开放许可**：CC0 / CC BY / CC BY-SA / 公有领域）
**判定红线**：只采用作者已明确开放许可的图（CC0 / CC BY / CC BY-SA / PD）——这类图许可本身就允许带署名再分发，可放心入库；**全权保留 / 厂商(Cadence/Synopsys) / 教材 / 付费或 non-CC 期刊（含 arXiv 默认 nonexclusive、BSD/GPL 软件图）一律不用**。每张都要**在来源页核验许可**（Wikimedia 的 File 页、论文的 license 声明），再下载使用。
...
**插入 + 标注**：
- 单图：`split(..., figure="ext_*.png", caption="图注：这张画了啥", credit="作者 · arXiv:xxxx / Wikimedia · 许可")`；`caption=`→「图 N · …」图注，`credit=`→图下「来源：…」小字。
