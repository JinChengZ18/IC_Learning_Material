...
**内联强调**：正文 / 表说明里用 `**重点**` → 渲成**加粗 + 强调色（橙）**（`deck._emph_segs`，仅 pptx；预览去标记）。专用来点亮**总分结构的中心短句**（一句一处，别整段加粗）。**标题 / sub 里出现术语缩写（ILM / PDN / EM / PG …）时，正文里该术语的「中英对照」也加粗**作寻路锚点——让标题缩写在正文一眼可定位；与中心短句强调可并存（一条正文可两处 `**…**`，如 4.3「**电迁移（EM）**是**……**」）。
