...
- **不滥用分点（多次返工的坑）**：**体裁对齐笔记**——笔记是叙述段就用 `style="prose"`（整段），**只有笔记本身是列表/枚举/对照表时才分点/表格**（并列 ▪、有序 ①②③、对照 `table`）。改一遍往往还不够，宁可多转 prose：Floorplan 里 1.3/2.1/2.2/2.3/3.2/3.3/4.1/4.2 等叙述小节都是 prose。
...
- 多图：`split(..., figs=[fg(路径, 图注, 来源), fg(...)])`——右栏并排多图、各带图注（`deck._split_figs`/`_psplit_figs`，1–3 张；选**长宽比相近**的图配对才好看）。
