...
## 配色（单一真源）
只改 `theme.py` 的 `BLUE/TEAL/AMBER/ROSE/VIOLET`（各带 `_L`/`_D`）+ `SOFT` + `CARD_EDGE`，跑一次 `figures.py` 即全局换肤（**变量名不变、只改 hex**）。`deck.PRIMARY/ACCENT = T.BLUE/T.AMBER.lstrip('#')`，chrome 自动跟随。配色随用户口味变过多轮（清华紫→鲜明多色→…→当前**深清华紫 `#5B2C86`** 主色），别假设是某一版——以 `theme.py` 现值为准。**用纯色、别滥用透明度**（卡片底用 `_L` 实色）。
...
程序画的是示意图；真实照片/版图截图/IR 颜色图等需引自文献：
1. `python tools/extract_figs.py <lecture.pdf>` → 提取**嵌入位图** → `lit/<pdf名>/`（已 gitignore）+ `manifest.csv`。
2. 挑合适的，在 `slides.py` 用 `split(..., figure="lit/<…>.png", credit="来源：Teman, DVD Lec6 (after Cadence)")`。
3. deck 末尾放 `refs` 页列参考文献。
**版权**：提取图版权属原始来源（讲义本身也转引 Cadence/Rabaey/CMOS-VLSI-Design 等）；**仅供个人学习、使用处必标来源、勿随仓库分发**（`lit/` 已 gitignore，别把版权图提交进库）。
