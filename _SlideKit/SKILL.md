...
**分工**：`.md` 教案是**纯文字文本源**（不嵌图）；图只活在 PPT 里（`figures.py` 自绘或文献真图）。slides 的内容**严格按笔记**，逐小节对应。
...
**内联强调**：正文 / 表说明里用 `**重点**` → 渲成**加粗 + 强调色（橙）**（`deck._emph_segs`，仅 pptx；预览去标记）。专用来点亮**总分结构的中心短句**（一句一处，别整段加粗）。
...
- **纯表格页别只有表**：笔记里表前后的引言 / 定义 / 结论用 `tbl(..., note="…")` 补进来——按**正文字号**渲染（不是小灰脚注），排在**表格上方**（顶左起、说明在前、表在后，符合阅读流）；`deck._table` 自动缩表留位。表格列 / 行按笔记给足，别留大片空白。
