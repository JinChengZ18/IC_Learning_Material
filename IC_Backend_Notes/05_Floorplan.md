...
本章承接上一章的几何与空间约束：当设计规模继续膨胀，问题不再是「单颗芯片怎么摆」，而是「整颗芯片还能不能一次跑完」。如果设计太大，**扁平流程（flat flow）**一次跑完整颗芯片会遇到运行时间（runtime）和内存压力。应对办法是**层次化设计（hierarchical design）**：把全芯片切成多个模块，每个模块独立做布局布线（P&R），再在顶层做全芯片时序和验证。

但切块不是免费的——它把一个「大问题」换成一组「边界问题」，因此 3.2 的时序预算、3.3 的引脚分配与穿通才成为层次化设计的关键。先看它的收益与代价：
